...
def make_pep(state, log=None, model="m1", role="support", control="C1"):
    ...
    pep.begin_task(TaskSpec(task_id="t1", task_type="process_refund", instruction="", ticket_id="T0001"))
    return pep


@pytest.mark.parametrize("action,params,detail", [
    ("issue_refund", '{"order_id": "O0001", "amount": ', "not valid JSON"),
    ("issue_refund", "[1, 2]", "must be a JSON object"),
    ("issue_refund", {"order_id": "O0001"}, "amount"),  # missing field
    ("issue_refund", {"order_id": "O0001", "amount": "lots"}, "amount"),  # wrong type
    ("issue_refund", {"order_id": "O0001", "amount": 5, "note": "x"}, "note"),  # extra field
...
def test_raw_json_string_arguments_are_parsed(state):
    pep = make_pep(state)
    reply = pep.gateway().call("lookup_order", '{"order_id": "O0001"}')
    e = pep.events[-1]
    assert reply["ok"] and not e.format_error and e.params == {"order_id": "O0001"}


def test_unparseable_raw_text_is_kept_in_params(state):
    pep = make_pep(state)
    pep.gateway().call("lookup_order", "order O0001 please")
    assert pep.events[-1].params == {"_raw": "order O0001 please"}
...
def test_format_errors_do_not_count_as_prior_calls(state):
    ...
    order = state.tickets["T0001"].order_ids[0]
    pep = make_pep(state)
...
def test_format_errors_excluded_from_detector_view(state, tmp_path):
    ...
    pep.gateway().call("lookup_order", {"order_id": "O0001"})
...
def test_format_error_rate_per_model(state, tmp_path):
    ...
    for params in ({"ticket_id": "T0001"}, {}, "{bad", {"ticket_id": "T0001"}):
        a.gateway().call("read_ticket", params)
    b.gateway().call("read_ticket", {"ticket_id": "T0001"})
